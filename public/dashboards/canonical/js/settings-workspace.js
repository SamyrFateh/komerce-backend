/**
 * @komerce-arch
 * @role          canonical-settings-workspace
 ...
 * @impact-areas  admin-dashboard, settings, config, finance-config
 * @version       2026-09
 */

'use strict';
/**
 * KOMERCE Admin Canonical — Paramètres (KomerceCanonicalSettingsWorkspace)
 * ════════════════════════════════════════════════════════════════════════
 * Porte la fonctionnalité de l'ancienne SettingsView (Legacy 1) dans le
 * shell Canonical, sans dépendance au shell Legacy ni à son api-client.js.
 * Même contrat d'API serveur, appelée ici en fetch direct (mêmes routes,
 * mêmes payloads) — voir docs/admin-nav-capability-map.md §7.
 *
 * 4 onglets :
...
(function (global) {
  ...
  /* ── API (fetch direct, credentials cookie httpOnly) ──────────────────── */
  const BASE_API = '/api';
  let _root = null; // référence stable au root passé à render() — jamais de requête DOM par id fixe
  ...
  const SettingsApi = {
    getSettings: () => _fetchJSON(`${BASE_API}/admin/rules`),
    getSettingRule: key => _fetchJSON(`${BASE_API}/admin/rules/${encodeURIComponent(key)}`),
  ...
  async function _openRulePanel(key) {
    ...
    try {
      const data = await SettingsApi.getSettingRule(key);
  ...
  async function _saveRule() {
    ...
    try {
      await SettingsApi.patchSettingRule(_editingRule.key, { value, reason });
      _closePanel();
      const root = _root;
  ...
  async function _resetRule() {
    ...
    try {
      await SettingsApi.resetSettingRule(_editingRule.key);
      _closePanel();
      const root = _root;
      const fresh = await SettingsApi.getSettings();
      _data.rules = fresh.categories;
  ...
  async function _renderAuditTab(container) {
    ...
    try {
      const data  = await SettingsApi.getSettingsAudit();
  ...
  async function render(root) {
    ...
    try {
      const [rulesData, taxesData, dimsData] = await Promise.all([
        ...
        SettingsApi.getSettingsTaxes(),
        SettingsApi.getSettingsDims(),
      ]);
      _data = {
  ...
  global.KomerceCanonicalSettingsWorkspace = Object.freeze({ render });
