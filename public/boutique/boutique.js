...
(function () {
  ...
  */
  ...
  /* ── STRIPE (P0.4 — clé chargée depuis /api/public/config) ─── */
  ...
  async function _loadPublicConfig() {
    ...
    try {
      ...
      _publicConfig = await r.json();
    } catch (e) {
      console.warn('[config] fallback : Stripe désactivé', e);
      _publicConfig = { stripe_public_key: '', eur_kmf_rate: 492, aed_kmf_rate: 138 };
    }
    return _publicConfig;
  }

  async function _ensureStripe() {
    if (_stripe) return _stripe;
    if (typeof Stripe === 'undefined') return null;
    const cfg = await _loadPublicConfig();
    if (!cfg.stripe_public_key) {
      console.warn('[stripe] aucune clé publique configurée côté serveur');
      return null;
    }
    try {
      _stripe = Stripe(cfg.stripe_public_key);
    } catch (e) { console.warn('[stripe] init failed:', e); return null; }
    return _stripe;
  }

  // Charger la config dès le boot (mais ne pas bloquer)
  _loadPublicConfig();
...
function quickRemove(productId, btnEl) {
  ...
  function renderCheckout() {
    ...
    /* ── 2bis. Point relais (P0.3 — obligatoire) ── */
    const sR = document.createElement('div');
    sR.className = 'ck-label';
    sR.textContent = '🏪 Point relais (obligatoire)';
    body.appendChild(sR);

    // Île
    const islandWrap = document.createElement('div');
    islandWrap.className = 'ck-field';
    islandWrap.innerHTML =
      '<label for="k-island-select" class="ck-field-label">Île de livraison *</label>' +
      '<select id="k-island-select" class="ck-input" required>' +
        '<option value="">— Choisir une île —</option>' +
        '<option value="grande_comore">Grande Comore</option>' +
        '<option value="anjouan">Anjouan</option>' +
        '<option value="moheli">Mohéli</option>' +
        '<option value="mayotte">Mayotte</option>' +
      '</select>';
    body.appendChild(islandWrap);

    // Relais (filtré par île)
    const relaisWrap = document.createElement('div');
    relaisWrap.className = 'ck-field';
    relaisWrap.innerHTML =
      '<label for="k-relais-select" class="ck-field-label">Point relais *</label>' +
      '<select id="k-relais-select" class="ck-input" required disabled>' +
        '<option value="">— D\'abord choisir une île —</option>' +
      '</select>' +
      '<div class="ck-help" id="k-relais-help" style="font-size:12px;color:#64748b;margin-top:4px;">Sélectionnez un relais pour calculer la livraison.</div>';
    body.appendChild(relaisWrap);

    // Préselection si od.island/relais_id déjà choisi
    const islandSel = islandWrap.querySelector('#k-island-select');
    const relaisSel = relaisWrap.querySelector('#k-relais-select');
    const relaisHelp = relaisWrap.querySelector('#k-relais-help');

    function fillRelaisForIsland(islandKey) {
      relaisSel.innerHTML = '';
      if (!islandKey) {
        relaisSel.innerHTML = '<option value="">— D\'abord choisir une île —</option>';
        relaisSel.disabled = true;
        if (relaisHelp) relaisHelp.textContent = 'Sélectionnez un relais pour calculer la livraison.';
        return;
      }
      const list = (state.relais || []).filter(r => {
        const rIsland = (r.island || r.zone || '').toLowerCase();
        return rIsland === islandKey || rIsland.indexOf(islandKey) >= 0;
      });
      if (!list.length) {
        relaisSel.innerHTML = '<option value="">Aucun relais disponible sur cette île</option>';
        relaisSel.disabled = true;
        if (relaisHelp) relaisHelp.textContent = 'Aucun relais actif. Contactez Komerce sur WhatsApp.';
        return;
      }
      relaisSel.disabled = false;
      relaisSel.innerHTML = '<option value="">— Choisir un relais —</option>' +
        list.map(r => '<option value="' + r.id + '">' +
          (r.name || r.label || r.id) +
          (r.address ? ' — ' + r.address : '') +
          '</option>').join('');
      if (od.relais_id) relaisSel.value = od.relais_id;
      if (relaisHelp) relaisHelp.textContent = list.length + ' relais sur ' +
        (islandKey === 'grande_comore' ? 'Grande Comore' :
         islandKey === 'anjouan' ? 'Anjouan' :
         islandKey === 'moheli' ? 'Mohéli' : 'Mayotte');
    }

    if (od.island) { islandSel.value = od.island; fillRelaisForIsland(od.island); }
    islandSel.addEventListener('change', e => {
      od.island = e.target.value || null;
      fillRelaisForIsland(od.island);
      od.relais_id = null;
      state.selectedRelaisId = null;
    });
    relaisSel.addEventListener('change', e => {
      od.relais_id = e.target.value || null;
      state.selectedRelaisId = od.relais_id;
    });
    ...
    function updatePaymentUI() {
      ...
      if (isStripe && !_stripeCard) {
        _ensureStripe().then((s) => {
          if (!s) {
            const errEl = document.getElementById('stripe-card-error');
            if (errEl) { errEl.textContent = 'Paiement carte indisponible. Essayez le paiement en cash relais.'; errEl.classList.add('is-visible'); }
            return;
          }
...
async function submitOrder(btn) {
  ...
  // P0.3 — Relais obligatoire (la route + le coût en dépendent)
  const selectedRelaisId = od.relais_id || state.selectedRelaisId;
  if (!selectedRelaisId) {
    showToast('Sélectionnez un point relais avant de commander.', 'error');
    // Faire défiler vers le sélecteur si présent
    const sel = document.getElementById('k-relais-select');
  ...
  try {
    ...
    if (isStripe) {
      ...
      const stripeReady = await _ensureStripe();
      if (!stripeReady) {
        showToast('Paiement carte indisponible. Réessayez ou choisissez le cash relais.', 'error');
        btn.disabled = false;
        btn.textContent = '✅ Confirmer';
        return;
      }
      const stripeResult = await stripeReady.confirmCardPayment(intentResult.client_secret, {
        payment_method: {
          card: _stripeCard,
          billing_details: {
            name: clientName,
  ...
  function setupDrawer() {
    ...
    // P1.1 — Bouton "Faire participer la famille" injecté dans le footer
    if (dom.cartFooter && !document.getElementById('k-cart-event-btn')) {
      const eventBtn = document.createElement('button');
      eventBtn.id = 'k-cart-event-btn';
      eventBtn.type = 'button';
      eventBtn.className = 'k-cart-event-btn';
      eventBtn.innerHTML = '🎉 Faire participer ma famille';
      eventBtn.style.cssText =
        'width:100%;padding:12px;margin-top:8px;border:1px solid #b5d4f4;' +
        'background:#e6f1fb;color:#0c447c;font-weight:600;font-size:14px;' +
        'border-radius:8px;cursor:pointer;';
      eventBtn.addEventListener('click', launchEventFromCart);
      // Insérer juste après le bouton checkout
      if (dom.cartCheckout && dom.cartCheckout.parentElement) {
        dom.cartCheckout.parentElement.appendChild(eventBtn);
      } else {
        dom.cartFooter.appendChild(eventBtn);
      }
    }
  ...
  /* P1.1 — Lance la création d'un workspace pré-rempli avec le panier */
  async function launchEventFromCart() {
    if (!state.cart || state.cart.length === 0) {
      showToast('Ajoutez d\'abord des articles à votre panier.', 'info');
      return;
    }
    // Sauvegarde du panier actuel pour pré-remplissage post-création
    try {
      const snapshot = state.cart.map(it => ({
        product_id: String(it.product.id),
        product_name: it.product.name,
        price_kmf: Number(it.product.price_kmf) || 0,
        quantity: Number(it.qty) || 1,
      }));
      sessionStorage.setItem('komerce_event_pending_cart', JSON.stringify(snapshot));
    } catch (_) { /* ignore */ }
    closeCart();
    // Redirection vers création (la page lira sessionStorage)
...
(function adaptivePlaceholder() {
  ...
  // ══════════════════════════════════════════════════════════
  // DEBUG BUTTON (P0.5 — UNIQUEMENT en dev)
  // Active si : localhost OU localStorage.KM_DEBUG = '1'
  // ══════════════════════════════════════════════════════════
  (function() {
    var isLocalhost = /^(localhost|127\.0\.0\.1|0\.0\.0\.0|\[::1\])$/.test(location.hostname) ||
                      location.hostname.endsWith('.local');
    var hasDebugFlag = false;
    try { hasDebugFlag = localStorage.getItem('KM_DEBUG') === '1'; } catch(_) {}
    if (!isLocalhost && !hasDebugFlag) return; // ← prod = pas de bouton debug
