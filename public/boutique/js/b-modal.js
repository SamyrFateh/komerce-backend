...
bus.on('modal:close', function() { if (typeof closeModal === 'function') closeModal(); });
  ...
  /**
   * Vague 1.4 — Calcule un message de délai de livraison pour un produit.
   * Priorité :
   *   1. product.delivery_estimate (string libre rempli côté admin/backend)
   *   2. produit local Comores (is_local / origin === 'local') → 2-3 jours
   *   3. fallback par catégorie (artisanat = rapide, mode/électronique = lent)
   *   4. fallback générique
   * Retourne null si rien de pertinent à afficher.
   ...
   * @returns {string|null}
   */
  function computeDeliveryEstimate(product) {
    if (!product) return null;
    // 1. Champ explicite (futur backend ou saisie admin)
    if (typeof product.delivery_estimate === 'string' && product.delivery_estimate.trim()) {
      return product.delivery_estimate.trim();
    }
    // 2. Produit local Comores (même artisan, livraison rapide possible)
    if (product.is_local || product.origin === 'local' || product.origin === 'comores') {
      return 'Reçu sous 2 à 3 jours à Moroni';
    }
    // 3. Fallback par catégorie. Les libellés en minuscules pour comparer sans accent.
    var cat = (product.category || '').toString().toLowerCase();
    // Catégories artisanales / locales : délai plus court attendu
    if (cat.indexOf('artisan') >= 0 || cat.indexOf('maison') >= 0 ||
        cat.indexOf('cuisine') >= 0 || cat.indexOf('alimentaire') >= 0 ||
        cat.indexOf('beauté') >= 0 || cat.indexOf('beaute') >= 0) {
      return 'Reçu sous 5 à 10 jours';
    }
    // Mode, électronique, accessoires : délai standard import
    if (cat.indexOf('mode') >= 0 || cat.indexOf('électron') >= 0 || cat.indexOf('electron') >= 0 ||
        cat.indexOf('high-tech') >= 0 || cat.indexOf('accessoire') >= 0 ||
        cat.indexOf('chaussure') >= 0 || cat.indexOf('vêt') >= 0 || cat.indexOf('vet') >= 0) {
      return 'Reçu sous 7 à 14 jours';
    }
    // 4. Générique
    return 'Reçu sous 7 à 10 jours';
  }

  /* ═════════════════════════════════════════════════════════════
     VAGUE 3 — UI variantes (taille / couleur / etc.)
     Frontend autonome : si product.variants est absent ou vide,
     l'UI reste invisible et le comportement est strictement
     identique à l'ancien (zéro régression).
     ═════════════════════════════════════════════════════════════ */

  /**
   * Lit les variantes d'un produit. Format attendu :
   *   product.variants = {
   *     "Taille":  [{ value:"M", stock:3, price_kmf:null, image_url:null }, ...],
   *     "Couleur": [{ value:"Bleu", stock:null, image_url:"...", price_kmf:null }, ...]
   *   }
   * Retourne {} si absent ou mal formé.
   */
  function readVariants(product) {
    if (!product || !product.variants || typeof product.variants !== 'object') return {};
    var clean = {};
    Object.keys(product.variants).forEach(function(key) {
      var arr = product.variants[key];
      if (Array.isArray(arr) && arr.length > 0) {
        clean[key] = arr.filter(function(v) { return v && v.value != null; });
  ...
  /**
   * Construit l'UI sélecteur (chips groupées par type de variante).
   * Initialise state.selectedVariants avec la première option en stock de chaque groupe.
   */
  function renderVariants(product) {
    var container = document.getElementById('k-modal-variants');
    if (!container) return;
    var variants = readVariants(product);
    var groupNames = Object.keys(variants);
    ...
    groupNames.forEach(function(groupName) {
      var options = variants[groupName];
      // Sélection par défaut : première option dont stock !== 0
      var firstAvailable = options.find(function(o) { return o.stock !== 0; }) || options[0];
      state.selectedVariants[groupName] = firstAvailable.value;

      html += '<div class="k-modal-variant-group" data-group="' + sanitize(groupName) + '">';
      html += '<div class="k-modal-variant-label">' + sanitize(groupName) + ' : ';
      html += '<strong class="k-modal-variant-selected" data-for="' + sanitize(groupName) + '">' +
              sanitize(firstAvailable.value) + '</strong></div>';
      html += '<div class="k-modal-variant-chips">';
      options.forEach(function(opt) {
        var isOut = opt.stock === 0;
        var isActive = opt.value === firstAvailable.value;
        var classes = 'k-modal-variant-chip' +
                      (isActive ? ' is-active' : '') +
                      (isOut ? ' is-out' : '');
        html += '<button type="button" class="' + classes + '"' +
                ' data-group="' + sanitize(groupName) + '"' +
                ' data-value="' + sanitize(opt.value) + '"' +
                (isOut ? ' aria-disabled="true"' : '') +
                '>' + sanitize(opt.value) + '</button>';
      });
      html += '</div></div>';
    });
    ...
    // Délégation click sur les chips
    container.addEventListener('click', _onVariantChipClick, false);
  }

  function _onVariantChipClick(e) {
    var chip = e.target.closest('.k-modal-variant-chip');
    if (!chip) return;
    ...
    // Mise à jour visuelle des chips du groupe
    var container = document.getElementById('k-modal-variants');
    container.querySelectorAll('.k-modal-variant-chip[data-group="' + group + '"]')
      .forEach(function(c) { c.classList.toggle('is-active', c.getAttribute('data-value') === value); });
    var label = container.querySelector('.k-modal-variant-selected[data-for="' + group + '"]');
    if (label) label.textContent = value;

    // Recalcule le stock de la combo et met à jour le badge stock + bouton acheter
    applyVariantSelection();
  }

  /**
   * Recalcule le stock effectif de la combo sélectionnée et met à jour :
   *   - le badge stock (k-modal-stock)
   *   - l'image principale si la variante choisie a une image_url
   *   - l'état du bouton "Acheter" (désactivé si combo en rupture)
   * Le stock effectif = min des stocks individuels (chaque variante peut limiter).
   * Si une variante a stock=null, elle ne contraint pas (= "non géré par cette variante").
   */
  function applyVariantSelection() {
    ...
    // Calcul du stock effectif
    var stocks = [];
    Object.keys(state.selectedVariants).forEach(function(group) {
      var val = state.selectedVariants[group];
      var opt = (variants[group] || []).find(function(o) { return o.value === val; });
      if (opt && typeof opt.stock === 'number') stocks.push(opt.stock);
    });
    // Si aucun stock contraint → on retombe sur product.stock global
    var effectiveStock = stocks.length > 0 ? Math.min.apply(null, stocks) : Number(product.stock || 0);

    // Update badge stock
    if (dom.modalStock) {
      if (effectiveStock === 0) {
        dom.modalStock.textContent = '✗ Rupture pour cette combinaison';
        dom.modalStock.className = 'k-modal-stock k-modal-stock--out';
      } else if (effectiveStock <= 10) {
        dom.modalStock.textContent = '🔥 Plus que ' + effectiveStock + ' en stock';
    ...
    // Désactive le bouton "Acheter" et "Ajouter" si rupture combo
    var buyBtn = document.getElementById('k-buy-now-btn');
    var addBtn = document.getElementById('k-add-cart-btn');
    [buyBtn, addBtn].forEach(function(b) {
      if (!b) return;
      if (effectiveStock === 0) {
    ...
    // Swap image principale si la variante choisie en a une
    Object.keys(state.selectedVariants).forEach(function(group) {
      var val = state.selectedVariants[group];
      var opt = (variants[group] || []).find(function(o) { return o.value === val; });
      if (opt && opt.image_url) {
        var firstSlide = document.querySelector('.k-modal-slide');
        if (firstSlide) {
          firstSlide.src = optimizeImgUrl(opt.image_url, 800);
          // Reset carousel à 0 pour montrer l'image variante
   ...
   */
    ...
    // ── Vague 1.4 : encart délai estimé livraison ─────────────
    // Priorité : champ produit explicite > flag local > fallback catégorie.
    // On masque l'encart si stock = 0 (rupture, le délai n'a pas de sens).
    var deliveryEl = document.getElementById('k-modal-delivery');
    var deliveryText = document.getElementById('k-modal-delivery-text');
    if (deliveryEl && deliveryText) {
      var msg = computeDeliveryEstimate(product);
      if (stockVal === 0 || !msg) {
    ...
    // ── Vague 3 : sélecteur de variantes (taille / couleur) ───
    // Invisible si product.variants n'existe pas. Si présent, initialise
    // state.selectedVariants et synchronise stock + image principale.
