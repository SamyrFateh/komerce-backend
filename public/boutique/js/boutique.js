/**
 * @module boutique
 * @brief Logique applicative Komerce boutique — en cours de migration ES modules.
 *
 * Phase 2 (Option C) : IIFE retirée → module ES.
 * Les blocs §1 KUtils et §2 STATE/DOM sont remplacés par des imports.
 *
 * §1  UTILS         → b-utils.js (imports directs)
 * §2  STATE & DOM   → b-store.js (state, SUBCATS, dom, initDom, $, $$)
 * §3  TOAST & CART CORE  ← commence ici
 * §4  CATALOG
 * §5  FLAT SUBCAT
 * §6  GRID SECTIONS
 * §7  CART INTERACTIONS
...
// Enregistré UNE SEULE FOIS — hors readyState check (modules = toujours deferred)
...
// Exposition de state sur window pour les handlers inline legacy
// (backward compat — à nettoyer en Phase 3)
if (typeof window !== 'undefined') window.state = state;

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() {
    initDom(); // Phase 2 : cache DOM refs depuis b-store.js
    init();
  });
} else {
  // Modules sont toujours deferred — DOM est prêt
  initDom();
  init();
}
