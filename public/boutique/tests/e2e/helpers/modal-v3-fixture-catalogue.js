...
async function expectCanonicalOrder(page) {
  const order = await page.evaluate(() => {
    ...
    const children = zone ? Array.from(zone.children) : [];
    return {
      conf: children.indexOf(conf),
      details: children.indexOf(details),
      enriched: children.indexOf(enriched),
      suggestions: children.indexOf(suggestions),
    };
  });
  expect(order.conf).toBeGreaterThanOrEqual(0);
  expect(order.conf).toBeLessThan(order.details);
  expect(order.details).toBeLessThan(order.enriched);
  expect(order.enriched).toBeLessThan(order.suggestions);
}
