...
test('rend le fallback vide d’une sous-catégorie sans produits', () => {
...
test('initialise le pager et synchronise la page visible au scroll', () => {
  const grid = mount();
  _setupFlatSubcatPager();
...
test('le swipe tactile horizontal déplace la grille, le vertical est ignoré', () => {
  const grid = mount();
  _setupFlatSubcatPager();
  grid.scrollLeft = 100;
...
test('les boutons ne sont pas volés par le swipe', () => {
  const grid = mount();
  _setupFlatSubcatPager();
...
test('le drag pointeur déplace la grille et neutralise le clic après swipe', () => {
  const grid = mount();
  _setupFlatSubcatPager();
  grid.scrollLeft = 50;
...
test('IntersectionObserver ajoute des produits et câble les cartes ajoutées', () => {
...
test('la fin de page propose la suivante et la dernière reste idempotente', () => {
...
test('les chips déléguées gèrent mobile, Tout et desktop', () => {
  mount();
