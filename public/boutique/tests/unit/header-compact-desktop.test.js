 ...
 * @purpose       G-05 — entre 900 et 1279px, les 4 boutons texte du header
...
describe('header compact desktop (G-05, 900–1279px)', () => {
  test('media query scopée 900–1279px présente', () => {
  ...
  test('sélecteur scopé .k-header-actions .k-header-nav-btn passe en icône 40x40', () => {
    expect(css).toMatch(/@media \(min-width: 900px\) and \(max-width: 1279px\)[\s\S]*\.k-header-actions \.k-header-nav-btn \{[\s\S]*flex: 0 0 40px;/);
  ...
  test('les libellés texte sont masqués dans ce media, pas le badge/icône', () => {
    expect(css).toMatch(/@media \(min-width: 900px\) and \(max-width: 1279px\)[\s\S]*\.k-header-actions \.k-header-nav-btn > span:not\(\.k-komerce-nav-icon, \.k-header-group-badge\),\s*\n\s*\.k-header-actions \.k-header-nav-btn \.k-komerce-nav-label \{\s*\n\s*display: none;/);
  });

  test('ne touche pas le CTA "Suivre ma commande" (sélecteur non scopé .k-header-nav-btn > span)', () => {
    // Le correctif G-05 ne doit exister que scopé à .k-header-actions ; il ne
    // doit jamais y avoir de règle masquant .k-header-nav-btn > span hors de
    // ce scope (qui casserait le CTA hero, qui porte aussi .k-header-nav-btn
    // mais pas .k-header-actions).
    const g05Block = css.slice(css.indexOf('@media (min-width: 900px) and (max-width: 1279px)'));
