...
/**
 * ============================================================
 * KOMERCE DASHBOARDS — Audit Gate (Niveau 1 — Pyramide Qualité)
 ...
 * (même principe que scripts/.code-quality-baseline.json pour le N2) :
...
function run() {
  ...
  const depMeta   = report.metadata && report.metadata.dependencies || {};
  ...
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║  KOMERCE DASHBOARDS — Audit Gate (N1 — dépendances)      ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');

  console.log(`  Dépendances auditées : ${(meta.total) || '?'}`);
  console.log(`  Vulnérabilités totales (toutes sévérités) : ${meta.total ?? '?'} (info: low ${meta.low ?? 0}, moderate ${meta.moderate ?? 0}, high ${meta.high ?? 0}, critical ${meta.critical ?? 0})`);
