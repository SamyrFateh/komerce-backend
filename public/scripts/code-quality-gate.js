...
/**
 * ============================================================
 * KOMERCE DASHBOARDS — Code Quality Gate (Niveau 2 — Pyramide Qualité)
...
// ── Configuration ─────────────────────────────────────────────────────────────
const SCAN_DIRS = ['dashboards/admin/js', 'dashboards/admin-legacy/js', 'js'];
const ROOT_FILES = ['sw.js'];

const IGNORE_PATTERNS = [
  /node_modules/,
  /\.min\.js$/,
  /admin-legacy/,  // status: deprecated dans le registre — dette gelee, pas de modernisation prevue
];
...
function printHelp() {
  console.log(`
╔══════════════════════════════════════════════════════════╗
║  KOMERCE DASHBOARDS — Code Quality Gate (N2)        ║
╚══════════════════════════════════════════════════════════╝
...
const RULE_FILE_EXEMPT = {
  'N2-NO-CONSOLE': new Set([
    'sw.js',  // Service Worker — console.log est le pattern standard pour debug SW
  ]),
  'N2-NO-VAR': new Set([
    // js/auth-guard.js et js/parcel-components.js : code partage hub/relais,
    // gele en l'etat — modernisation prevue lors de la refonte platform.
    'js/auth-guard.js',
...
const RULES = [
  ...
  // ── N2.6 — SQL concaténé ──────────────────────────────────────────────────
  {
    id: 'N2-SQL-INJECTION', // desactive pour dashboards (pas de SQL frontend)
  ...
  // ── N2.7 — Route async sans try/catch ─────────────────────────────────────
  {
    ...
    fixable: false,
    check(src, lines, rel) {
      return []; // dashboards n'a pas de routes Express
      const hits = [];
...
function main() {
  ...
  if (args.json) {
    ...
    console.log('\n╔══════════════════════════════════════════════════════════╗');
    console.log('║  KOMERCE DASHBOARDS — Code Quality Gate (N2)        ║');
    console.log('╚══════════════════════════════════════════════════════════╝\n');
