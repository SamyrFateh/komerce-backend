...
(function () {
  ...
  */
  ...
  // Expose SUBCATS pour le moteur Bazar vivant
  if (typeof window !== "undefined") window.SUBCATS = SUBCATS;
...
// ═══════════════════════════════════════════════════════════════════════
//  🌺 BAZAR VIVANT — Home rendering
//  ─────────────────────────────────────────────────────────────────────
//  Génère une home en mini-sections adaptatives selon le stock.
//  Contrainte R&D : équilibre tradition (sous-cats nommées) × modernité.
// ═══════════════════════════════════════════════════════════════════════
(function() {
  // ── CURATIONS saisonnières (configurable) ────────────────────────
  const CURATIONS = [
    { theme: 'maman',    icon: '🎁', label: 'Pour Maman',      filter: null },
    { theme: 'ramadan',  icon: '🕌', label: 'Ramadan 2026',    filter: null },
    { theme: 'diaspora', icon: '🔥', label: 'Top diaspora',    filter: null },
    { theme: 'bebe',     icon: '👶', label: 'Tout bébé',       filter: (p) => p.category === 'Enfant' },
    { theme: 'comores',  icon: '🇰🇲', label: 'Fait aux Comores', filter: null },
  ];

  // ── NARRATIFS (rotation aléatoire) ───────────────────────────────
  const NARRATIVES = [
    {
      type: 'origin',
      label: '✨ Saviez-vous ?',
      content: "Nos boubous de cérémonie sont confectionnés par des couturières d'Anjouan qui travaillent le tissu pendant jusqu'à 3 semaines."
    },
    {
      type: 'testimonial',
      label: '💬 Témoignage',
      content: '"J\'ai envoyé 3 boubous depuis Paris à ma cousine pour son mariage. Tout est arrivé en 9 jours, nickel."',
      author: '— Fatima M., Paris 🇫🇷 → Mutsamudu'
    },
    {
      type: 'saveur',
      label: '🌺 Le saviez-vous',
      content: "L'ylang-ylang des Comores parfume <strong>70% des grands parfums français</strong>. Chanel n°5 en contient depuis 1921."
    },
    {
      type: 'astuce',
      label: '💡 Astuce livraison',
      content: "Commandez avant le 15 du mois pour recevoir avant la fin du mois sur Anjouan et Mohéli. Express à Grande Comore."
    },
    {
      type: 'testimonial',
      label: '💬 Retour client',
      content: '"Ma mère a pleuré en recevant le parfum ylang. 20 ans qu\'elle n\'en avait pas senti."',
      author: '— Youssouf B., Marseille → Moroni'
    },
    {
      type: 'origin',
      label: '🧵 Artisan Komerce',
      content: "Nos pochettes tressées traditionnelles sont réalisées par les artisanes du village de Bweni sur Mohéli, technique transmise de mère en fille depuis 3 générations."
    },
  ];
  ...
  function renderTrustBar() {
    return (
      '<div class="k-trust-bar">' +
        '<span class="k-trust-item">🇰🇲 Livré en 10j</span>' +
        '<span class="k-trust-item">🛡 Cash livraison</span>' +
        '<span class="k-trust-item">⭐ 4.8/5</span>' +
      '</div>'
    );
  }
  ...
  function renderCurations() {
    var items = CURATIONS.map(function(c) {
      return (
        '<button class="k-curation-card" data-theme="' + c.theme + '" data-curation="' + c.theme + '">' +
          '<span class="k-curation-icon">' + c.icon + '</span>' +
          c.label +
        '</button>'
      );
    }).join('');
    return (
      '<div class="k-curations-section">' +
        '<div class="k-curations-rail">' + items + '</div>' +
      '</div>'
    );
  }

  // ── MINI-CARD (produit compact pour rail) ────────────────────────
  function renderMiniCard(p) {
    var price = formatPrice(p.price_kmf || 0);
    var oldPrice = '';
    if (p.promo_pct) {
      var old = Math.round((p.price_kmf || 0) / (1 - p.promo_pct / 100));
      oldPrice = '<span class="k-mini-card-price-old">' + formatPrice(old) + '</span>';
    }
    var promoBadge = p.promo_pct
      ? '<span class="k-mini-card-promo">-' + p.promo_pct + '%</span>'
      : '';
    var fav = (window.state && window.state.favs && window.state.favs.indexOf(String(p.id)) > -1) ? '❤️' : '🤍';
    var img = p.image_url || p.image || '';
    return (
      '<div class="k-mini-card" data-id="' + p.id + '" onclick="openProductModal(\'' + p.id + '\')">' +
        '<div class="k-mini-card-img-wrap">' +
          promoBadge +
          '<button class="k-mini-card-fav" data-fav="' + p.id + '" onclick="event.stopPropagation();toggleFav(\'' + p.id + '\', this)">' + fav + '</button>' +
          (img ? '<img src="' + img + '" alt="' + escapeHtml(p.name || '') + '" loading="lazy">' : '<span style="font-size:48px">🧺</span>') +
        '</div>' +
        '<div class="k-mini-card-info">' +
          '<div class="k-mini-card-name">' + escapeHtml(p.name || '') + '</div>' +
          '<div class="k-mini-card-price">' + price + oldPrice + '</div>' +
        '</div>' +
      '</div>'
    );
  }
  ...
  // Adaptatif selon le stock : < 3 produits → pas de section
  function renderMiniSection(title, emoji, products, seeAllHandler) {
    if (!products || products.length < 3) return '';  // Seuil minimum
    var cards = products.slice(0, 12).map(renderMiniCard).join('');
    var seeAllAttr = seeAllHandler ? ' onclick="' + seeAllHandler + '"' : '';
    return (
      '<div class="k-mini-section">' +
        '<div class="k-mini-section-header">' +
          '<div class="k-mini-section-title">' +
            '<span class="k-mini-section-title-emoji">' + emoji + '</span>' +
            escapeHtml(title) +
            '<span class="k-mini-section-count">· ' + products.length + ' articles</span>' +
          '</div>' +
          '<button class="k-mini-section-more"' + seeAllAttr + '>Voir tout →</button>' +
        '</div>' +
        '<div class="k-mini-section-rail">' + cards + '</div>' +
      '</div>'
    );
  }
  ...
  function renderNarrative(n) {
    var authorHtml = n.author ? '<div class="k-narrative-author">' + escapeHtml(n.author) + '</div>' : '';
    return (
      '<div class="k-narrative" data-type="' + n.type + '">' +
        '<div class="k-narrative-label">' + n.label + '</div>' +
        '<div class="k-narrative-content">' + n.content + '</div>' +
        authorHtml +
      '</div>'
    );
  }

  // ── MAIN : renderBazarHome() ─────────────────────────────────────
  // Construit la home complète Bazar vivant
  window.renderBazarHome = function() {
    var container = document.getElementById('k-bazar-home');
    if (!container) {
      // Créer le container si pas encore là
      container = document.createElement('div');
      container.id = 'k-bazar-home';
      // L'insérer APRÈS les catégories, avant la grille
      var catsWrap = document.getElementById('k-cats-wrap') || document.querySelector('.k-cats-shell') || document.querySelector('.k-cats');
      if (catsWrap && catsWrap.parentNode) {
        catsWrap.parentNode.insertBefore(container, catsWrap.nextSibling);
      } else {
        document.body.appendChild(container);
      }
    }

    // Récupérer les produits depuis l'état global
    var state = window.state || {};
    var allProducts = state.products || [];
    if (!allProducts.length) {
      container.innerHTML = renderTrustBar() + '<div style="padding:40px 20px;text-align:center;color:#888">Chargement de la boutique...</div>';
      return;
    }

    // Filtrer par catégorie active si nécessaire
    var activeCat = state.activeCat || 'all';
    var filtered = (activeCat === 'all' || activeCat === 'Tout') ? allProducts : allProducts.filter(function(p) { return p.category === activeCat; });

    // Grouper par sous-catégorie
    var bySubcat = {};
    filtered.forEach(function(p) {
      var sub = p.subcategory || 'Autres';
      if (!bySubcat[sub]) bySubcat[sub] = [];
      bySubcat[sub].push(p);
    });

    // Récupérer l'ordre "traditionnel" des sous-cats si défini, sinon ordre alpha
    var SUBCATS_MAP = window.SUBCATS || {};
    var subcatOrder = [];
    if (activeCat !== 'all' && activeCat !== 'Tout' && SUBCATS_MAP[activeCat]) {
      // Mode tradition : ordre défini dans SUBCATS
      subcatOrder = SUBCATS_MAP[activeCat].map(function(s) { return s.key; });
    } else {
      // Mode "Tout" : ordre par richesse de stock (les plus fournies d'abord)
      subcatOrder = Object.keys(bySubcat).sort(function(a, b) {
        return (bySubcat[b] ? bySubcat[b].length : 0) - (bySubcat[a] ? bySubcat[a].length : 0);
      });
    }

    // Emoji par sous-cat (fallback)
    var emojiBySubcat = {};
    Object.keys(SUBCATS_MAP).forEach(function(cat) {
      (SUBCATS_MAP[cat] || []).forEach(function(sub) {
        emojiBySubcat[sub.key] = sub.icon || '📦';
      });
    });
    ...
    // Trust bar + curations (toujours en haut)
    html += renderTrustBar();
    html += renderCurations();

    // Mini-sections, avec narratifs tous les 2-3 rails
    var miniSectionCount = 0;
    var narrativeIdx = Math.floor(Math.random() * NARRATIVES.length);
    
    subcatOrder.forEach(function(subKey, i) {
      var prods = bySubcat[subKey];
      if (!prods || prods.length < 3) return;  // Seuil minimum (adaptatif)

      var emoji = emojiBySubcat[subKey] || '🛍';
      html += renderMiniSection(subKey, emoji, prods, 'filterBySubcat(\'' + subKey + '\')');
      miniSectionCount++;

      // Injecter narratif tous les 2 sections (et pas à la fin)
      if (miniSectionCount % 2 === 0 && i < subcatOrder.length - 1) {
        html += renderNarrative(NARRATIVES[narrativeIdx % NARRATIVES.length]);
        narrativeIdx++;
      }
    });

    // Si aucune mini-section n'a passé le seuil
    if (miniSectionCount === 0) {
      html += '<div style="padding:40px 20px;text-align:center;color:#888">';
      html += '<div style="font-size:36px;margin-bottom:8px">🔍</div>';
      html += '<div>Aucun produit dans cette catégorie pour l\'instant.</div>';
      html += '</div>';
    }

    container.innerHTML = html;

    // Activer le mode bazar sur body
    document.body.classList.add('bazar-mode');

    console.log('[BAZAR] Rendu home :', miniSectionCount, 'mini-sections pour cat=' + activeCat);
  };

  // Helper : toggleFav fallback si pas défini ailleurs
  if (typeof window.toggleFav === 'undefined') {
    window.toggleFav = function(pid, btn) {
      if (!window.state || !window.state.favorites) return;
      var idx = window.state.favs.indexOf(String(pid));
      if (idx > -1) {
        window.state.favs.splice(idx, 1);
        btn.textContent = '🤍';
      } else {
        window.state.favs.push(String(pid));
        btn.textContent = '❤️';
      }
      // Persister
      try { localStorage.setItem('k_favs', JSON.stringify(window.state.favs)); } catch(e) {}
      if (typeof window.saveFavorites === 'function') window.saveFavorites();
    };
  }

  // Helper : filterBySubcat (tap "Voir tout →" d'une mini-section)
  if (typeof window.filterBySubcat === 'undefined') {
    window.filterBySubcat = function(subKey) {
      if (window.state) {
        window.state.activeSubcat = subKey;
        document.body.classList.remove('bazar-mode');
  ...
  // Helper : openProductModal fallback
  if (typeof window.openProductModal === 'undefined') {
    window.openProductModal = function(pid) {
      if (typeof window.openModal === 'function') window.openModal(pid);
    };
  }

  // ── INTÉGRATION : activer le mode bazar au chargement ────────────
  // Quand la home charge, on affiche bazar ; quand user tape une cat spécifique,
  // bazar affiche les mini-sections de cette cat ; quand user tape une sous-cat,
  // on bascule en mode grille classique.
  function tryRenderBazar() {
    // Attendre que state.products soit chargé (tableau non vide)
    if (window.state && Array.isArray(window.state.products) && window.state.products.length > 0) {
      window.renderBazarHome();
    } else {
      // Réessayer plus tard (le state n'est pas encore prêt)
      setTimeout(tryRenderBazar, 250);
    }
  }

  // Délai minimal pour laisser le reste du JS se charger
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      setTimeout(tryRenderBazar, 100);
    });
  } else {
    setTimeout(tryRenderBazar, 100);
  }
  ...
  document.addEventListener('click', function(e) {
    var chip = e.target.closest('.k-chip[data-cat]') || e.target.closest('.k-cat-chip');
    if (chip) {
      // Donner un petit délai pour que le state soit mis à jour
      setTimeout(function() {
        if (window.state) {
          var activeCat = chip.dataset.cat || chip.getAttribute('data-cat');
          window.state.activeCat = activeCat;
          window.state.activeSubcat = null;
          window.renderBazarHome();
        }
      }, 50);
    }
  ...
  // Expose les curations pour le back / admin
  window.KOMERCE_CURATIONS = CURATIONS;
  window.KOMERCE_NARRATIVES = NARRATIVES;
})();



// ═══════════════════════════════════════════════════════════════════════
//  🌺 BAZAR VIVANT — Toggle & Safety
//  ─────────────────────────────────────────────────────────────────────
//  Permet de désactiver le mode bazar via localStorage si besoin (debug).
//  Dans la console : localStorage.setItem('k_bazar_mode', 'off') + reload
//  Par défaut : activé.
// ═══════════════════════════════════════════════════════════════════════
(function bazarSafety() {
  var bazarMode = localStorage.getItem('k_bazar_mode') !== 'off';
  if (!bazarMode) {
    console.log('[BAZAR] Mode désactivé via localStorage (k_bazar_mode=off)');
    // Court-circuite renderBazarHome si appelée
    window.renderBazarHome = function() {
      console.log('[BAZAR] Mode off — rendu classique');
      document.body.classList.remove('bazar-mode');
      var container = document.getElementById('k-bazar-home');
      if (container) container.innerHTML = '';
    };
  }
  // Helpers console pour toggler rapidement
  window.komerceBazarOn = function() {
    localStorage.setItem('k_bazar_mode', 'on');
    location.reload();
  };
  window.komerceBazarOff = function() {
    localStorage.setItem('k_bazar_mode', 'off');
    location.reload();
  };
})();
