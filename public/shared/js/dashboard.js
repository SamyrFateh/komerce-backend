...
router.get('/clients/list', async (req, res, next) => {
  try {
    ...
    // Construction dynamique des paramètres (uniquement ceux qui sont VRAIMENT utilisés)
    const params = [];
    let pi = 0;
    const nextParam = (val) => { params.push(val); return ++pi; };

    let searchClause = '';
    if (search) {
      const idx = nextParam('%' + search.toLowerCase() + '%');
      searchClause = `(LOWER(client_name) LIKE $${idx} OR LOWER(client_phone) LIKE $${idx})`;
    }

    let segmentClause = '';
    if (segment === 'new') {
      segmentClause = `COUNT(*) = 1 AND EXTRACT(DAY FROM NOW() - MAX(created_at))::int <= 30`;
    } else if (segment === 'recurrent') {
      segmentClause = `COUNT(*) >= 2 AND EXTRACT(DAY FROM NOW() - MAX(created_at))::int <= 90`;
    } else if (segment === 'vip') {
      const idx = nextParam(seuilVipKmf);
      segmentClause = `(SUM(total_kmf) >= $${idx} OR COUNT(*) >= 5) AND EXTRACT(DAY FROM NOW() - MAX(created_at))::int <= 180`;
    } else if (segment === 'at_risk') {
      segmentClause = `COUNT(*) >= 2 AND EXTRACT(DAY FROM NOW() - MAX(created_at))::int BETWEEN 60 AND 180`;
    } else if (segment === 'dormant') {
      segmentClause = `EXTRACT(DAY FROM NOW() - MAX(created_at))::int > 180`;
    }

    const havingClauses = [searchClause, segmentClause].filter(Boolean);
    const havingSql = havingClauses.length ? 'HAVING ' + havingClauses.join(' AND ') : '';

    let islandWhereSql = '';
    if (island) {
      const idx = nextParam(island);
      islandWhereSql = `AND rl.island = $${idx}`;
    }
    ...
    const sql = `
      ...
      GROUP BY client_phone, client_name
      ${havingSql}
      ORDER BY ltv_kmf DESC NULLS LAST
      LIMIT ${pageSize} OFFSET ${offset}
    `;

    // Pour le count : exactement les mêmes params (mêmes filtres)
    const countSql = `
      WITH oc AS (
        SELECT
    ...
    res.json({
      ...
      clients: listRes.rows.map(c => ({
        name: c.name, phone: c.phone,
        nb_commandes: parseInt(c.nb_commandes),
        ltv_kmf: Math.round(parseFloat(c.ltv_kmf || 0)),
        panier_moyen_kmf: Math.round(parseFloat(c.panier_moyen_kmf || 0)),
        premiere_commande: c.premiere_commande,
        derniere_commande: c.derniere_commande,
        jours_silence: parseInt(c.jours_silence),
      })),
