...
describe('image snapshot fiable (Amendement V2 §C)', () => {
  it('image absente (null) -> pas de <img>, fallback affiché, conteneur en erreur', () => {
    activateSharedListContext(publicPayload({ items: [availableItem({ image: null })] }), 'tok-1');
    renderSharedListInCart();
    const wrap = document.querySelector('.k-cart-item-img');
    expect(wrap.classList.contains('is-img-error')).toBe(true);
    expect(wrap.querySelector('.k-cart-item-img-el')).toBeNull();
    expect(wrap.querySelector('.k-cart-item-img-fallback')).not.toBeNull();
  });

  it('image absente (chaîne vide) -> même fallback que null', () => {
    activateSharedListContext(publicPayload({ items: [availableItem({ image: '   ' })] }), 'tok-1');
    renderSharedListInCart();
    const wrap = document.querySelector('.k-cart-item-img');
    expect(wrap.classList.contains('is-img-error')).toBe(true);
    expect(wrap.querySelector('.k-cart-item-img-el')).toBeNull();
  ...
  it('URL invalide (schéma javascript:) -> traitée comme absente, jamais posée en src', () => {
    activateSharedListContext(
      publicPayload({ items: [availableItem({ image: 'javascript:alert(1)' })] }),
      'tok-1'
    );
    renderSharedListInCart();
    const wrap = document.querySelector('.k-cart-item-img');
  ...
  it('URL invalide (schéma présent mais hôte manquant) -> fallback, pas de <img>', () => {
    activateSharedListContext(
      publicPayload({ items: [availableItem({ image: 'http://' })] }),
      'tok-1'
    );
    renderSharedListInCart();
    const wrap = document.querySelector('.k-cart-item-img');
  ...
  it('URL http(s) valide -> <img> rendu via le pipeline optimizeImgUrl, fallback masqué par défaut', () => {
    activateSharedListContext(
      publicPayload({
        items: [availableItem({
          image: 'https://res.cloudinary.com/demo/image/upload/v1/riz.jpg',
        })],
      }),
      'tok-1'
    );
    renderSharedListInCart();
    const wrap = document.querySelector('.k-cart-item-img');
    expect(wrap.classList.contains('is-img-error')).toBe(false);
    const img = wrap.querySelector('.k-cart-item-img-el');
    expect(img).not.toBeNull();
    // Pipeline canonique b-utils.js::optimizeImgUrl — f_auto,q_auto,w_100 injecté pour Cloudinary.
    expect(img.src).toContain('/upload/f_auto,q_auto,w_100/');
    expect(wrap.querySelector('.k-cart-item-img-fallback')).not.toBeNull();
  });

  it('URL relative (chemin site) -> acceptée, résolue sur l\'origine', () => {
    activateSharedListContext(
      publicPayload({ items: [availableItem({ image: '/images/products/riz.jpg' })] }),
      'tok-1'
    );
    renderSharedListInCart();
    const wrap = document.querySelector('.k-cart-item-img');
    expect(wrap.classList.contains('is-img-error')).toBe(false);
    expect(wrap.querySelector('.k-cart-item-img-el')).not.toBeNull();
  });

  it("erreur de chargement (onerror) -> l'image casse proprement en fallback, pas d'icône cassée", () => {
    activateSharedListContext(
      publicPayload({
        items: [availableItem({ image: 'https://example.com/introuvable.jpg' })],
      }),
      'tok-1'
    );
    renderSharedListInCart();
