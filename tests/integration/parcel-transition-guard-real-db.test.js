...
if (!hasIntegrationEnv) {
  ...
  async function seedParcelInPreparation(label) {
    ...
    await db.query(
      `INSERT INTO parcels (id, order_id, reference, status, relais_id)
       VALUES ($1, $2, $3, 'preparation', $4)`,
      [parcelId, orderId, `${RUN_TAG}-parcel-${seedCounter}`, relaisId]
    );

    return { parcelId, orderId };
  }

  async function seedOpenIncident({ parcelId, orderId, incidentType, originDomain = 'LOGISTICS' }) {
    const id = crypto.randomUUID();
    incidentIds.push(id);
    await db.query(
      `INSERT INTO incidents (id, parcel_id, order_id, incident_type, status, title, origin_domain, resolver_domain, resolution_class)
       VALUES ($1, $2, $3, $4, 'open', $5, $6, $6, 'PHYSICAL_PROOF')`,
      [id, parcelId, orderId, incidentType, `E2E F2 guard test — ${incidentType}`, originDomain]
    );
    return id;
  }
  ...
  afterAll(async () => {
    if (incidentIds.length) {
      // incidents est protégé par un trigger de suppression (RAISE
      // EXCEPTION, « Utilisez status=dismissed ») — même contournement
      // que pour parcels plus bas, déjà établi ailleurs dans le
      // codebase (tests/e2e-api/products.kmf-numeric.e2e.test.js).
  ...
  describe('services/parcel-transition-guard.js (F2) — via transitionParcelStatus réel (REAL_DB)', () => {
    ...
    it("5 — REPLI order_id : un incident rattaché seulement à la commande (parcel_id NULL) bloque quand même", async () => {
      ...
      await db.query(
        `INSERT INTO incidents (id, parcel_id, order_id, incident_type, status, title, origin_domain, resolver_domain, resolution_class)
         VALUES ($1, NULL, $2, 'damaged_item', 'open', $3, 'LOGISTICS', 'LOGISTICS', 'PHYSICAL_PROOF')`,
