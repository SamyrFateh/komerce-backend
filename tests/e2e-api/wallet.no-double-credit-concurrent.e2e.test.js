...
describeE2E('E2E-P1-WALLET — no double credit/debit concurrent', ({ db }) => {
  ...
  async function runContendedPair(userId, operation) {
    ...
    try {
      ...
      // Ne libérer la ligne que lorsque les DEUX transactions ont déjà passé
      // leur SELECT idempotence et attendent le verrou wallet.
  ...
  beforeAll(async () => {
    ...
    // LIFO : lots -> transactions -> wallets -> users.
    ...
    // Pré-créer la ligne du crédit à 0 pour que les deux opérations attendent
    // exactement le même verrou de ligne, sans mêler la course de lazy-create.
    await db.query('INSERT INTO wallets (user_id, balance_kmf) VALUES ($1, 0)', [creditUserId]);

    // Pour le débit, créer un vrai lot de 10 000 KMF via le service canonique.
