...
describe('collective-workspace-creation', () => {
  ...
  it('cree un workspace conception avec tokens bruts retournes une seule fois', async () => {
    ...
    expect(mockInternals.logEvent).toHaveBeenCalledWith(client, 'ws-001', 'workspace_created', 'creator', 'sam@example.com', {
  ...
  it('rollback si insertion DB echoue', async () => {
    const client = makeClient([{ error: new Error('db_down') }]);
    mockInternals.db.pool.connect.mockResolvedValue(client);
