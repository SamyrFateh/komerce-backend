...
describe('collective-workspace-reads', () => {
  ...
  it('getWorkspaceByPublicToken retourne null si le public token est inconnu', async () => {
    mockInternals.db.query.mockResolvedValueOnce({ rows: [] });

    await expect(getWorkspaceByPublicToken('WS-token')).resolves.toBeNull();
    expect(mockInternals.db.query).toHaveBeenCalledWith(expect.stringContaining('public_token_hash = $1'), ['hash:WS-token']);
  });
  ...
  it('getWorkspaceByPublicToken assemble workspace, items, contributions et session active', async () => {
    ...
    expect(mockInternals.db.query.mock.calls[2][0]).toContain("status != 'cancelled'");
    expect(mockInternals.db.query.mock.calls[3][0]).toContain("status IN ('open','ready_to_capture')");
  });
  ...
  it('getWorkspaceByCreatorToken retourne la ligne complete du createur', async () => {
    ...
    mockInternals.db.query.mockResolvedValueOnce({ rows: [workspace] });

    await expect(getWorkspaceByCreatorToken('WC-token')).resolves.toBe(workspace);
    expect(mockInternals.db.query).toHaveBeenCalledWith(expect.stringContaining('creator_token_hash = $1'), ['hash:WC-token']);
  });
