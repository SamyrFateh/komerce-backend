...
describe('collective-workspace-lifecycle', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    internals._generateToken.mockReturnValue('PT-token-001');
  ...
  describe('finalizationReview', () => {
    it('calcule total, intentions, gap et can_finalize', async () => {
      ...
      expect(internals.logEvent).toHaveBeenCalledWith(null, 'ws-001', 'finalization_reviewed', 'creator', null, expect.any(Object));
    });
  ...
  describe('resumeWorkspace', () => {
    it('reprend une session terminee en conception', async () => {
      ...
      expect(client.calls[1].sql).toContain("SET status = 'conception'");
      expect(client.calls[2].sql).toContain("SET status = 'intention'");
      expectTransactionCommitted(client);
    });
