...
describe('AliExpress purchase preflight', () => {
  ...
  test('fail-closed si l’identité commandable manque', () => {
    ...
    expect(() => preflight.resolveOrderableUnit(c, 'AE-SKU-RED-M', 1)).toThrow(/supplier_order_identity requis/i);
  });

  test('refuse une identité d’un autre fournisseur', () => {
    const c = contract();
    c.sellable_units[0].supplier_order_identity.provider = 'cj';
    expect(() => preflight.resolveOrderableUnit(c, 'AE-SKU-RED-M', 1)).toThrow(/incompatible avec AliExpress/i);
  });
});
