...
} = require('../../scripts/schema-promote');
...
describe('schema-promote column-level live proof', () => {
  test('parses columns under their exact live table', () => {
    const parsed = parseLiveColumnsByTable(liveSchema(['inventory_model', 'name_source'], ['sku_id']));
  ...
  test('does not accept a same-named column that exists under another table', () => {
  ...
  test('promotes only when the target table contains every migration column', () => {
    const resolveMigration = () => [{
      name: '104_product_skus.sql',
      sql: `
