...
describe('wallet-receipt', () => {
  ...
  it('genere une reference WAL et persiste un snapshot metadata fige', async () => {
    const tx = {
      ...
      amount_kmf: 5000, direction: 'credit', reason: 'refund', note: 'annulation',
      order_id: 'order-001', lot_id: 'lot-001',
    };
    ...
    expect(documentService.persistDocument).toHaveBeenCalledWith(expect.objectContaining({
      ...
      metadata: expect.objectContaining({
        ...
        amount_kmf: 5000, direction: 'credit', reason: 'refund', note: 'annulation', order_id: 'order-001', lot_id: 'lot-001',
      }),
      dbClient: pool,
    }));
  });
  ...
  it('utilise dbClient fourni sans passer par pool pour query/persist', async () => {
    const dbClient = { query: jest.fn()
      .mockResolvedValueOnce({ rows: [{ id: 'tx-001', reason: 'loyalty', user_id: 'u1', amount_kmf: 1000, direction: 'credit' }] })
