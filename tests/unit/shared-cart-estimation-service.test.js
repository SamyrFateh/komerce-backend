...
describe('shared-cart-estimation-service', () => {
  ...
  it('upsertEstimation cree une estimation si aucun telephone existant', async () => {
    ...
    expect(client.calls[3].sql).toContain('INSERT INTO shared_cart_estimations');
    expect(client.calls[4].params[1]).toBe('estimation_created');
  ...
  it('upsertEstimation met a jour lestimation existante par telephone', async () => {
    ...
    expect(client.calls[3].sql).toContain('UPDATE shared_cart_estimations');
    expect(client.calls[4].params[1]).toBe('estimation_updated');
    expectTransactionCommitted(client);
  });
  ...
  it('deleteEstimation supprime avec garde telephone optionnelle', async () => {
    ...
    expect(client.calls[2].params).toEqual(['est-001', 'cart-001', '+269000']);
    expect(client.calls[3].params[1]).toBe('estimation_deleted');
    expectTransactionCommitted(client);
  });
