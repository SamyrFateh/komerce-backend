...
describe('shared-cart-contributions', () => {
  ...
  it('startContribution cree une contribution pending et un event', async () => {
    ...
    expect(client.calls[1].sql).toContain('INSERT INTO shared_cart_contributions');
  ...
  it('markContributionFailed marque failed et journalise si une ligne est retournee', async () => {
    ...
    expect(client.calls[0].sql).toContain("SET status = 'failed'");
    expect(client.calls[1].sql).toContain('INSERT INTO shared_cart_events');
    expectTransactionCommitted(client);
  });
});
