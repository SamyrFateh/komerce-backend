...
describe('shared-cart-commitment-service', () => {
  ...
  it('createOrUpdateCommitment cree un engagement pledged et un evenement', async () => {
    ...
    expect(client.calls[2].sql).toContain('INSERT INTO shared_cart_commitments');
    expect(client.calls[2].params).toEqual(['cart-001', 'Ali', '+269000', 5000, 'ok', JSON.stringify({ source: 'public_shared_cart' })]);
    expect(client.calls[3].params[1]).toBe('commitment_created');
  ...
  it('createOrUpdateCommitment met a jour lengagement existant par telephone', async () => {
    ...
    expect(client.calls[2].sql).toContain('UPDATE shared_cart_commitments');
    expect(client.calls[3].params[1]).toBe('commitment_updated');
    expectTransactionCommitted(client);
  });
  ...
  it('withdrawCommitment retire avec garde telephone et evenement', async () => {
    ...
    expect(client.calls[1].params).toEqual(['commit-001', 'cart-001', '+269000', JSON.stringify({ reason: 'changed' })]);
    expect(client.calls[2].params[1]).toBe('commitment_withdrawn');
    expectTransactionCommitted(client);
  });
