-- @migration 219_market_delegation_decision_signal_manage_live.sql
...
-- Future assignments inherit the capability from the current template.
...
-- Existing active assignments receive the capability in their ceiling.
...
-- Promote only the existing manager-shaped memberships. Viewers and field
-- agents are not widened implicitly.
WITH eligible AS (
...
SELECT NULL,
       ...
       NULL,
       jsonb_build_object('capability', 'decision_signal.manage', 'source', 'migration-219'),
       NOW(),
       'migration-219'
  FROM inserted i
