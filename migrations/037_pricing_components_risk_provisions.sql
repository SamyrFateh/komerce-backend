-- ============================================================
-- Migration 037: Pricing Components + Risk Provisions (Étape 2)
-- Date: avril 2026
--
-- OBJECTIF MÉTIER:
--   Rendre le module Pricing pleinement extensible avec 2 nouvelles tables :
--
--   1. pricing_components : variables de coût par commande (Niveau 1 du calcul)
--      - Sourcing, Transit, Douane, Hub, Distribution, Paiement
--      - Chaque ligne a un toggle is_active pour activation/désactivation
--      - Ajout/modification/suppression possible (sauf composants système)
--
--   2. risk_provisions : provisions risques en % (Niveau 3 du calcul)
--      - Retours, casse, impayés cash, démarque, compensations
--      - Idem CRUD complet + toggle
--
-- POLITIQUE :
--   - Composants système (is_editable=false) : valeur modifiable, label/key non
--   - Composants utilisateur (is_editable=true) : tout modifiable, suppression libre
--   - Soft delete par défaut (is_active=false) + hard delete avec confirmation
-- ============================================================
...
CREATE TABLE IF NOT EXISTS pricing_components (
  ...
  key TEXT UNIQUE NOT NULL,             -- 'agent_fee', 'fret_eur_m3', ...
  label TEXT NOT NULL,                  -- "Commission agent achat"
  emoji TEXT,                           -- '🤝'
  category TEXT NOT NULL,               -- 'sourcing'|'transit'|'douane'|'hub'|'distribution'|'paiement'
  default_value NUMERIC NOT NULL,       -- 5.0
  unit TEXT NOT NULL,                   -- 'pct'|'kmf'|'kmf_per_kg'|'kmf_per_m3'|'aed'|'aed_per_unit'
  applies_to TEXT NOT NULL DEFAULT 'all', -- 'all'|'channel:diaspora'|'channel:cash_relais'|'category:phones'|...
  is_active BOOLEAN NOT NULL DEFAULT TRUE,    -- ⭐ TOGGLE ON/OFF
  is_editable BOOLEAN NOT NULL DEFAULT TRUE,  -- false = composant système (label/key verrouillés)
  is_deletable BOOLEAN NOT NULL DEFAULT TRUE, -- false = composant système (suppression interdite)
...
-- ── 2. Table risk_provisions (Niveau 3 — provisions % sur chaque cmd) ──────

CREATE TABLE IF NOT EXISTS risk_provisions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  key TEXT UNIQUE NOT NULL,             -- 'returns', 'unpaid_cash', ...
  label TEXT NOT NULL,                  -- "Retours produits defectueux"
  emoji TEXT,                           -- '\u21A9'
  rate_pct NUMERIC NOT NULL,            -- 1.5 = 1.5% du prix vente
  applies_to TEXT NOT NULL DEFAULT 'all', -- 'all'|'channel:cash_relais'|'category:cosmetiques'|...
  is_active BOOLEAN NOT NULL DEFAULT TRUE,    -- ⭐ TOGGLE ON/OFF
...
-- ── 3. Enrichir charges (Niveau 2) avec emoji + display_order ──────────────
...
-- ── 4. Seed pricing_components (composants SYSTEME — récupère le simulateur)
...
INSERT INTO pricing_components (
  ...
  -- Sourcing (verrouillés - composants système)
  ...
  -- Douane (système - les valeurs réelles viennent de customs_categories)
  ('transitaire_pct', 'Commission transitaire', E'\U0001F4DD', 'douane',
    2.0, 'pct', 'all', TRUE, TRUE, FALSE, 10,
    'Commission du transitaire pour le dedouanement (% sur valeur CIF)'),
  ...
  ('hub_controle_kmf', 'Controle qualite hub', E'\U00002705', 'hub',
    ...
    'Cout par commande pour controle qualite a la reception au hub'),
  ...
  ('commission_relais_kmf', 'Commission relais', E'\U0001F4B0', 'distribution',
    ...
    'Commission versee au relais pour la remise au client (KMF par cmd)'),
...
INSERT INTO risk_provisions (
  ...
  ('unpaid_cash', 'Impayes cash relais', E'\U0001F4B5', 3.0, 'channel:cash_relais',
    TRUE, TRUE, FALSE, 20,
    'Provision pour commandes payees en cash au relais mais non encaissees finalement'),
  ...
  ('compensation', 'Compensations qualite', E'\U0001F381', 0.5, 'all',
    TRUE, TRUE, FALSE, 50,
    'Provision pour gestes commerciaux compensant problemes qualite ou retard')

ON CONFLICT (key) DO NOTHING;

-- ── 6. Vérification (à exécuter après migration) ──────────────────────────
