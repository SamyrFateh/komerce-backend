-- @migration 223_market_delegation_decision_signal_manage_live.sql
...
-- Promote only existing manager-shaped memberships. Viewers and field agents
-- are not widened implicitly.
WITH eligible AS (
...
SELECT NULL,
       ...
       NULL,
       jsonb_build_object('capability', 'decision_signal.manage', 'source', 'migration-223'),
       NOW(),
       'migration-223'
  FROM inserted i
