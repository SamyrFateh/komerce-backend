-- ============================================================
-- 169 — Pricing market decision policy events
-- ============================================================
-- Objet : matérialiser la politique canonique qui gouverne la fenêtre du gate
-- économique d'un marché et ses seuils de décision, sans fallback caché.
--
-- 168 appartient déjà à market-autonomy (prix commercial local).
...
CREATE TABLE IF NOT EXISTS pricing_market_decision_policy_events (
  ...
  maturity_threshold NUMERIC(7,6) NOT NULL CHECK (maturity_threshold >= 0 AND maturity_threshold <= 1),
  coverage_threshold NUMERIC(12,6) NOT NULL CHECK (coverage_threshold > 0),
  max_disposition_ratio NUMERIC(7,6) NOT NULL CHECK (max_disposition_ratio >= 0 AND max_disposition_ratio <= 1),
  disposed_contribution_treatment TEXT NOT NULL DEFAULT 'EXCLUDE_FROM_NUMERATOR'
  ...
  CONSTRAINT pricing_market_decision_policy_version_unique UNIQUE (market_id, version),
  CONSTRAINT pricing_market_decision_policy_start_unique UNIQUE (market_id, effective_from)
);
