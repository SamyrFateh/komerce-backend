...
| `markets` | _ambiguë_ | no-declared-writer | — | local-stock, providers-services |
...
- backend : 958 fichier(s) `.js`/`.mjs` observés (canal A)
...
- `providers-services` → `market` (déclaré : `market (référentiel markets — lecture seule)`)
