...
2. Disposer d'une autorisation OAuth **Komerce Golden**, application différente de `Komerce Sandbox Sourcing` connectée à Railway. Le premier run a pu faire tourner son refresh token et l'enregistrer seulement dans la DB éphémère, désormais détruite. **Avant tout nouveau run, obtenir par device_code un nouveau refresh token de l'application Golden et remplacer `KOMERCE_ALLEGRO_PROOF_REFRESH_TOKEN` dans GitHub Actions** ; ne pas toucher au token de Railway, ne jamais le coller dans un ticket/chat.
...
Ce jalon n'installe pas de service Railway, n'exécute pas de cron, n'achète rien et n'utilise aucun token ou donnée client de Railway. Un seul run manuel, une référence exacte, jusqu'à 5 GET fournisseur (baseline + 4 vérifications). L'autorisation Golden peut à nouveau faire tourner son refresh token ; pour un futur run il faudra éventuellement la renouveler localement et mettre à jour **uniquement** son secret GitHub dédié.
