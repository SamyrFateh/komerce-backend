...
| MTN MoMo Congo | Un seul POST d'authentification Collections sur le Sandbox MTN, avec trois secrets dédiés. | **Aucun** RequestToPay, opération sur wallet, paiement ou réconciliation ; P0/P1 concerne exclusivement le token Sandbox. Voir `docs/external-providers/payments/MTN_MOMO_CG.md`. |
...
- Secrets MTN : `KOMERCE_MTN_PROOF_COLLECTION_SUBSCRIPTION_KEY`, `KOMERCE_MTN_PROOF_API_USER`, `KOMERCE_MTN_PROOF_API_KEY` — credentials Collections **Sandbox dédiés**, distincts du compte production et des secrets Railway. Lancer `providers=mtn-momo-cg`, `mode=sandbox-read` pour un unique POST d'authentification. Il n'y a aucun test de paiement dans cette campagne.
...
## Observation indépendante de la campagne GitHub (2026-09-22)

L'opérateur a exécuté une sonde ponctuelle **sur l'instance Railway existante**, avec les variables Sandbox qui y étaient déjà configurées, et partagé un rapport expurgé de **2 PASS / 0 BLOCKED** : authentification OAuth + lecture de la configuration webhook PayPal, et authentification Collections MTN. Voir [preuve et périmètre exacts](../_archive/external-provider-proofs/RAILWAY_SANDBOX_AUTH_2026-09-22.md). Aucune copie des secrets Railway dans GitHub n'a été nécessaire. Ce résultat, attesté par le rapport transmis par l'opérateur, **n'est pas** un artifact de la campagne GitHub : [run 35753825169](https://github.com/SamyrFateh/komerce-backend/actions/runs/35753825169) conserve son propre bilan de **0 PASS** lié à ses identifiants dédiés non disponibles. Ne pas fusionner les compteurs, ne pas promouvoir les fournisseurs globalement à P1/P2/P3/P4 et ne pas confondre authentification et paiement.
