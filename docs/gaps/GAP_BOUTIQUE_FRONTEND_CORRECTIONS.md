...
| **GAP-F4** | Budget de poids et cache | Moyenne | ⏳ À faire | — |
...
### Delta minimal (dans l'ordre, une mesure avant/après à chaque étape)
1. Cache long (`max-age=31536000, immutable`) sur les fichiers dont l'URL est versionnée ; `no-cache` sur le HTML. Vérifier l'interaction avec le service worker (`komerce-v341`).
2. Découper `components.css` par écran ; ne charger au démarrage que le CSS de l'accueil.
3. Charger au défilement les images hors du premier écran.
4. Identifier les scripts non nécessaires au premier affichage et les différer.

### Tests requis
Script de mesure reproductible (Playwright, viewport 390, réseau limité) archivé avec les chiffres avant/après.
