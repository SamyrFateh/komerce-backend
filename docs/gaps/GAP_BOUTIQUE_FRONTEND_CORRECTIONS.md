...
| **GAP-F4** | Budget de poids et cache | Moyenne | ✅ Fait (4/4 étapes) | — |
...
**2. Découper `components.css` par écran — ✅ Fait (approche différente de celle prévue).** Un découpage par nom de fichier s'est révélé dangereux dès la première vérification (`interactions.css` mélange navigation du bas critique et styles de panier différables). Remplacé par une extraction fondée sur une **mesure de couverture CSS réelle du navigateur** (Chrome DevTools Coverage via Playwright, accueil réel avec 8 produits sur 5 rayons, mobile + desktop, défilement complet) plutôt qu'un découpage manuel par fichier ou par nom de classe.

Résultat : seulement 5,2 à 7,3 % de `components.css` (507 Ko) est réellement utilisé pour afficher l'accueil. Ce sous-ensemble (273 règles CSS, extraites via `postcss` en préservant le contexte `@media` — une extraction par plages d'octets brutes casse le nesting, trouvé et corrigé en cours de route) devient `css/critical-home.css` (38 Ko), régénérable via `scripts/generate-critical-home-css.js`.

**Purement additif** : `components.css` reste intact, aucune règle n'en est retirée. Chargement : `critical-home.css` en `<link rel="stylesheet">` bloquant (petit, rapide), `components.css` en `<link rel="preload" as="style">` non bloquant, basculé vers `stylesheet` une fois téléchargé par un script externe dédié (`js/preload-css-swap.js`). Un `onload=` inline directement sur le `<link>` — le patron standard habituel pour cette technique — **aurait été silencieusement bloqué** par la CSP du site (`scriptSrcAttr: ["'none'"]`, `bootstrap/security.js`, FRESH-030) : trouvé en vérifiant la CSP réelle avant d'implémenter, pas après. L'ordre de cascade (`base` → `critical-home` → `components` → `desktop`) est préservé quelle que soit la vitesse de chargement, car le `<link>` `components.css` garde sa position dans le DOM — seul son `rel` change.

Testé en conditions réelles : accueil (mobile + desktop) et ouverture d'une modale produit (dépend du contenu différé) — 0 erreur JS, rendu visuel identique, captures vérifiées.

Deux garde-fous de gouvernance déjà existants ont réagi à raison à ce nouveau fichier généré (`critical-home.css` étant un miroir verbatim, pas une source sémantique indépendante) : `critical-selector-ownership.js` (contrat de propriété par sélecteur) l'excluait à tort de son scan — corrigé en l'excluant uniquement de ce contrat précis, pas de `listCss()` globalement (qui sert aussi l'inventaire de bundles, les résumés hex/`!important` et la détection de tokens cassés — l'exclure partout aurait cassé la vérité canonique des bundles, trouvé et corrigé en cours de route).

`check:css-guard` et `check:css-specificity-guard` remontent des conflits de cascade pré-existants (confirmés identiques sur `main` non modifié, par comparaison directe) entre `components.css` et `desktop.css` — sans rapport avec ce travail, non corrigés ici, et confirmé qu'aucun des deux outils n'est câblé dans le hook pre-commit ni dans la CI (ne bloquent donc rien).
