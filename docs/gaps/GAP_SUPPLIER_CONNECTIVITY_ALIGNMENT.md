...
### Ce que GAP-5 a réellement livré (PR #1606)

**Divergence spec/réalité assumée, plus importante que celle de GAP-4A/4B** : la spec originale ciblait le bloc `auto` de `purchasing-trigger-service.js` comme point de câblage de la réconciliation — écrite avant que GAP-4 n'établisse que ce bloc est inatteignable en production. Le vrai point vivant, trouvé en vérifiant plutôt qu'en suivant la spec à la lettre : `services/purchasing-admin-service.js:confirmPurchaseOrder()`, appelée par `POST /api/purchasing/:order_id/confirm` — la route qu'un humain utilise réellement après un achat manuel Allegro. Avant ce GAP, elle écrivait `data.supplier_order_id` sans aucune vérification.

- `provider-authority.js:reconciliationRequirement()` — nouvelle capability, axe indépendant de `remotePreflightRequirement` (un provider peut avoir l'un sans l'autre — AliExpress a un preflight REQUIRED mais une réconciliation NOT_REQUIRED, fait constaté, pas une lacune).
- `allegro-fulfillment-adapter.js:reconcile()` — câble enfin `allegro-purchase-reconciliation.js`, qui existait déjà mais n'était jamais attaché à l'objet adapter utilisé par le registry. Traduit `READY_FOR_PROCESSING` natif vers `commitment_verdict:'committed'` — le cœur ne voit jamais le statut natif.
- `purchase-order-confirmation-boundary.js` (nouveau) — frontière pure, minimum canonique figé (`provider, external_ref, commitment_verdict, evidence` opaque), aucun DTO riche.
- `confirmPurchaseOrder()` — appelle la frontière avant toute écriture. Rejet → `409`, aucune UPDATE tentée. Succès → persiste la `external_ref` vérifiée, jamais la valeur brute fournie.
- Non-régression totale pour tout provider hors Allegro (aucune réconciliation prouvée → comportement historique préservé).

### Leçons de GAP-5 — à lire avant GAP-6

**1. Une spec écrite tôt peut cibler le mauvais endroit une fois l'architecture réelle mieux comprise — vérifier où est le vrai point d'impact avant d'implémenter à la lettre.** La spec GAP-5 avait été écrite avant GAP-4A/4B et imaginait le bloc auto comme point de câblage. Le suivre à la lettre aurait livré une frontière techniquement correcte mais qui ne change RIEN au comportement réel de Komerce (le bloc auto n'est jamais atteint). **Toujours confirmer par du code réel, pas seulement par la spec, quel chemin est vivant avant de décider où câbler quoi que ce soit.**

**2. Le pattern de threading de `context` établi en GAP-4A s'est révélé directement réutilisable, exactement comme prédit par la leçon 3 de GAP-4.** `confirmPurchaseOrder(poId, orderId, data, options)` avec `options.context` transmis tel quel jusqu'à `adapter.reconcile()` — même seam, même discipline (jamais interprété par le cœur). Confirme que ce pattern est le bon choix par défaut chaque fois qu'un futur GAP a besoin d'injecter un test double au bout d'une chaîne d'appels.
