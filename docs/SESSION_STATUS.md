...
> **RÈGLE** : Ce fichier est mis à jour après chaque action significative et committé sur GitHub.
...
### Migration DB — Colonne `last_login_at`
- **Statut** : ⚠️ BLOQUÉ — en attente credentials Supabase
- **Action requise** : Exécuter dans Supabase SQL Editor :
  ```sql
  ALTER TABLE users ADD COLUMN IF NOT EXISTS last_login_at TIMESTAMPTZ;
  ```
- **Pourquoi** : La page `Komerce_Admin_Users.html` fait un SELECT sur cette colonne qui n'existe pas encore en DB
...
- **Credentials nécessaires** : URL projet Supabase + Service Role Key
...
- **Statut** : ✅ Terminé (sauf migration `last_login_at`)
- **Fichiers modifiés** :
...
- **Commits** : `8140656`
- **Principe** : Tout agent lit AGENT_RULES.md → CARTOGRAPHY_360.md → SESSION_STATUS.md avant tout code
