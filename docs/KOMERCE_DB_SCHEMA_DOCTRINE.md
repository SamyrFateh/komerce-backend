...
2. `node scripts/check-schema-freshness.js` — verifies that all columns declared in `migrations/*.sql` are present in the dump. Blocks if the dump is partial.
...
```bash
psql "$DATABASE_URL" -f docs/db/railway-live-schema.sql   # load full schema into throwaway Postgres
node scripts/ci-migrate.js                                 # dynamic baseline: only migrations added after last snapshot
```
...
# or locally if RAILWAY_DATABASE_URL is available:
npm run db:snapshot
