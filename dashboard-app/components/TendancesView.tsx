...
export const TendancesView: React.FC = () => {
  const { data: historyData, loading: l1, error: e1, reload: r1, usingMock: m1 } = useApi(() => api.history(), mockHistoryData, 15000);
  const { data: forecastData, loading: l2, error: e2, reload: r2, usingMock: m2 } = useApi(() => api.forecast(), mockForecastData, 15000);

  const loading = l1 || l2;
  const error = e1 || e2;
  const usingMock = m1 || m2;
  const reload = () => { r1(); r2(); };
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {!loading && historyData && forecastData && (
        <>
