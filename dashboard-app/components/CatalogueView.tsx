...
export const CatalogueView: React.FC = () => {
  const { data: pilotageData, loading: l1, error: e1, reload: r1, usingMock: m1 } = useApi(() => api.pilotage(), mockPilotageData, 15000);
  const { data: clientsData, loading: l2, error: e2, reload: r2, usingMock: m2 } = useApi(() => api.clients(), mockClientsData, 15000);
  const { data: financeData, loading: l3, error: e3, reload: r3, usingMock: m3 } = useApi(() => api.finance(), mockFinanceData, 15000);
  ...
  const categories = pilotageData!.categories;
  const pipeline = pilotageData!.pipeline;
  const topProduits = clientsData!.top_produits;
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {!loading && pilotageData && clientsData && financeData && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {categories.map((cat) => {
              ...
              const finCat = financeData!.par_categorie.find((fc) => fc.categorie === cat.categorie);
              return (
