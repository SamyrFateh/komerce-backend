...
module.exports = {
  ...
  classification: {
    ...
    rationale: [
      ...
      'Le handoff WhatsApp est un canal de conversation service-only, résolu côté serveur après identification Komerce et création de l Inquiry ; ' +
      'il ne devient ni une troisième intention Inquiry ni une exposition publique du numéro provider.',
  ...
  service: 'Porter l’identité d’un provider tiers, ses services et offres physiques, leur exposabilité, ' +
    ...
    'Pour un Service explicitement compatible WhatsApp, le serveur peut remettre un handoff de conversation après création de l Inquiry, ' +
    'sans publier le numéro dans le détail. Aucun paiement, settlement, calendrier structuré ou order Komerce n’est créé par cette interaction.',
  ...
  security: {
    ...
    note: 'GET /services/:id et GET /physical-offers/:id restent publics et minimaux : provider_name, image_ref et actions request/callback. ' +
      'Service peut en plus exposer whatsapp_available booléen, jamais une coordonnée. POST /inquiries est protégé par authenticateOrCreateGuest et le csrfOriginGuard global. ' +
      'Le requester_phone est exclusivement dérivé de req.user.phone. Un handoff WhatsApp service est résolu serveur depuis public_whatsapp seulement après exposabilité et identité ; ' +
      'la réponse peut alors contenir une URL wa.me liée à la nouvelle Inquiry, sur action explicite du client.',
  },
  ...
  authority: 'backend-core — providers-services possède le cycle demande/confirmation, la projection request/callback, ' +
    'le capability WhatsApp service, la résolution serveur du handoff après Inquiry, le nom public minimal du provider, les médias source service/physical_offer, ' +
    'le contexte Inquiry et ses données de démonstration staging ; recommendations ne fait que projeter ces lectures.',
  ...
  invariants: [
    ...
    { statement: 'la projection publique des intentions ne contient que request et/ou callback ; call/whatsapp historiques convergent vers callback et aucune coordonnée provider brute n est exposée',
      test: 'tests/unit/providers-interaction-policy.test.js' },
    { statement: 'un Service peut signaler whatsapp_available sans exposer le numéro ; le handoff WhatsApp est résolu serveur, exige une Inquiry et ne s applique jamais à physical_offer',
      test: 'tests/unit/providers-services-routes.test.js' },
    { statement: 'market est toujours résolu côté serveur avant les checks d’exposabilité',
      test: 'tests/unit/providers-services-routes.test.js' },
    { statement: 'POST /inquiries dérive requester_phone de la session authentifiée et accepte seulement intent request|callback ; whatsapp reste un handoff et non une intention',
      test: 'tests/unit/providers-services-routes.test.js' },
    { statement: 'request/callback sur une physical_offer restent des Inquiry providers-services et ne créent jamais une order Komerce ni un handoff WhatsApp',
  ...
  // 2026-09-16 — Service Detail dédié + handoff WhatsApp traçable après Inquiry ; aucune coordonnée brute exposée avant action explicite.
};
