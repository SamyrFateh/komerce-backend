 ...
 *
 * Les labels d'alerte terrain restent une projection UI ; la persistence
 * canonique utilise incidents.incident_type via incident-write-service.
 */
'use strict';
...
const AlertEngine = {
  async runAll() {
    ...
    const alerts = [];
    for (const r of results) if (r.status === 'fulfilled') alerts.push(...r.value);
    return alerts;
  },
  ...
  async _createAlertIfNew(type, parcelId, orderId, severity, description, metadata) {
    return createAlertEngineIncidentIfNew(db, { type, parcelId, orderId, severity, description, metadata });
  },
  ...
  async getActiveAlerts(filters) {
    ...
    const { rows } = await db.query(`
      SELECT i.*,
        COALESCE(i.details->>'alert_type', i.incident_type) AS type,
  ...
  async acknowledgeAlert(alertId, acknowledgedBy) {
    return acknowledgeAlertEngineIncident(db, alertId, acknowledgedBy);
  },
};
