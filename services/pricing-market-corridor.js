...
function contributionAt(point, variableCost) {
  const price = finiteNumber(point?.price_kmf);
  const cost = finiteNumber(variableCost);
  if (price == null || cost == null) return null;
  return rounded(price - cost);
}

function purchaseCeilings(priceKmf, economics = {}) {
  const price = finiteNumber(priceKmf);
  const outsidePurchase = finiteNumber(economics.variable_cost_outside_purchase_kmf);
  if (price == null || outsidePurchase == null) {
    return { break_even_kmf: null, safe_kmf: null, target_margin_kmf: null };
  }

  const safetyPct = Math.max(0, finiteNumber(economics.safety_margin_pct) || 0);
  const targetMarginPct = Math.min(99, Math.max(0, finiteNumber(economics.target_margin_pct) || 0));
  const variableSafe = safetyPct > 0 ? price / (1 + safetyPct / 100) : price;
  const variableTarget = price * (1 - targetMarginPct / 100);

  return {
    break_even_kmf: rounded(nonNegative(price - outsidePurchase)),
    safe_kmf: rounded(nonNegative(variableSafe - outsidePurchase)),
    target_margin_kmf: rounded(nonNegative(variableTarget - outsidePurchase)),
  };
}

function viabilityLabel(status) {
  return ({
    VIABLE: 'Viable',
...
function projectSkuViability(corridor = {}, economics = {}) {
  ...
  const variableCost = finiteNumber(economics.variable_cost_complete_kmf);
  const purchaseCost = finiteNumber(economics.purchase_cost_kmf);
  const minimumSafePrice = finiteNumber(economics.minimum_safe_price_kmf);
  const lowPrice = finiteNumber(corridor.low?.price_kmf);
  const targetPrice = finiteNumber(corridor.target?.price_kmf);
  const highPrice = finiteNumber(corridor.high?.price_kmf);
  const contributions = {
  ...
  return {
    ...
    variable_cost_outside_purchase_kmf: finiteNumber(economics.variable_cost_outside_purchase_kmf),
    minimum_safe_price_kmf: minimumSafePrice,
    market_prices_kmf: { low: lowPrice, target: targetPrice, high: highPrice },
    contribution_scenarios_kmf: contributions,
    purchase_cost_ceiling_at_target_kmf: ceilings,
    purchase_cost_gap_to_safe_ceiling_kmf: purchaseCost == null || ceilings.safe_kmf == null ? null : rounded(ceilings.safe_kmf - purchaseCost),
    purchase_cost_gap_to_target_margin_ceiling_kmf: purchaseCost == null || ceilings.target_margin_kmf == null ? null : rounded(ceilings.target_margin_kmf - purchaseCost),
