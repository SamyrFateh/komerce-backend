...
async function resolveMarketByCode(executor = db, codeValue) {
  ...
  const { rows } = await executor.query(
    `SELECT id, code, name, currency, minor_unit FROM markets
...
async function assertProductPriceShapeCompatible(executor = db, productId) {
  const { rows: [shape] } = await executor.query(
    `SELECT
       EXISTS (SELECT 1 FROM product_skus WHERE product_id = $1 AND is_active = TRUE AND price_kmf IS NOT NULL) AS has_explicit_sku_price,
       EXISTS (SELECT 1 FROM product_variants WHERE product_id = $1 AND price_kmf IS NOT NULL) AS has_explicit_variant_price`,
    [productId]
  );
...
async function resolveActiveProductMarketPricing(executor = db, { marketId, product }) {
  ...
  const { rows } = await executor.query(
    `SELECT d.amount, d.currency, d.status, d.active_at, m.currency AS market_currency
      ...
      WHERE d.market_id = $1::uuid AND d.product_id = $2::uuid AND d.status = 'LOCAL_ACTIVE'
      LIMIT 1`,
    [marketId, product.id]
  );
...
async function resolveActiveProductMarketPricingById(executor = db, { marketId, productId }) {
  ...
  const { rows: [product] } = await executor.query(
    `SELECT id, price_kmf, promo_pct, is_promo, promo_until FROM products
...
async function applyActiveMarketPricesToCatalogRows(executor = db, { marketCode, products }) {
  ...
  const { rows: decisions } = await executor.query(
    `SELECT product_id, amount, currency, active_at FROM product_market_price_drafts
      WHERE market_id = $1::uuid AND product_id = ANY($2::uuid[]) AND status = 'LOCAL_ACTIVE'`,
  ...
  const { rows: conflicts } = await executor.query(
    `SELECT DISTINCT product_id::text AS product_id FROM (
       SELECT product_id FROM product_skus WHERE product_id = ANY($1::uuid[]) AND is_active = TRUE AND price_kmf IS NOT NULL
       UNION ALL
       SELECT product_id FROM product_variants WHERE product_id = ANY($1::uuid[]) AND price_kmf IS NOT NULL
     ) granular`,
    [decisionIds]
  );
  if (conflicts.length) {
    throw new MarketCommercialPriceError(409, 'market_local_price_granular_price_conflict', 'Un prix pays actif entre en conflit avec un prix SKU/variante explicite.');
  }
  const output = [];
  for (const product of products) {
    ...
    output.push({
      ...
      market_price_promo_applied: effectiveUnitPriceKmf < baseUnitPriceKmf,
