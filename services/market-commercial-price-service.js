...
async function setMarketPriceDraft({ market, productRef, amount, reason, source, actorId }) {
  ...
  return withTransaction(async (client) => {
    ...
    await client.query(
      ...
      [market.id, product.id, existing ? existing.amount : null, localAmount, market.currency,
        existing ? existing.status : null, decisionReason, decisionSource, actorId || null]
    );

    return projectDecision({
      ...rows[0], product_ref: product.product_ref, name: product.name, category: product.category,
      global_price_kmf: product.price_kmf, decision_status: rows[0].status,
      decision_reason: rows[0].reason, decision_source: rows[0].source,
      decided_at: rows[0].updated_at, local_amount: rows[0].amount,
    }, market);
  });
}
...
async function authorizeMarketPriceDraft({ market, productRef, authorizationSnapshot, reason, source, actorId }) {
  ...
  return withTransaction(async (client) => {
    ...
    const { rows: lockedRows } = await client.query(
      `SELECT amount, currency, status FROM product_market_price_drafts
        WHERE market_id = $1::uuid AND product_id = $2::uuid FOR UPDATE`,
      [market.id, product.id]
    );
    const current = lockedRows[0];
    if (!current) throw new MarketCommercialPriceError(404, 'market_price_draft_not_found', 'Aucune décision locale à autoriser.');
    ...
    const { rows } = await client.query(
      `UPDATE product_market_price_drafts
          SET status = 'LOCAL_AUTHORIZED_PENDING_CUTOVER', authorized_at = NOW(),
              authorization_snapshot = $3::jsonb, decided_by = $4::uuid, updated_at = NOW()
    ...
    await client.query(
      ...
      [market.id, product.id, current.amount, current.currency, current.status,
        JSON.stringify(authorizationSnapshot), decisionReason, decisionSource, actorId || null]
    );
...
async function resetMarketPriceDraft({ market, productRef, reason, source, actorId }) {
  ...
  return withTransaction(async (client) => {
    ...
    if (!previous) throw new MarketCommercialPriceError(404, 'market_price_draft_not_found', 'Aucune décision locale à réinitialiser pour ce produit.');
    ...
    await client.query(
      ...
      [market.id, product.id, previous.amount, previous.currency, previous.status,
        previous.authorization_snapshot == null ? null : JSON.stringify(previous.authorization_snapshot),
        decisionReason, decisionSource, actorId || null]
    );

    return {
      product_ref: product.product_ref, market_code: market.code, currency: market.currency,
      previous_local_price: Number(previous.amount), previous_status: previous.status,
      decision_status: 'RESET_TO_GLOBAL_BASE', buyer_effective: false,
    };
  });
}
