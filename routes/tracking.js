 ...
 */
const STATUS_ORDER = [
  'ordered', 'confirmed', 'preparation', 'shipped',
...
router.get('/:token', async (req, res) => {
  try {
    ...
    // Fetch parcels with scan events
    ...
    // Fetch scan events for all parcels of this order
    ...
    // If no specific timestamps, use status to show current step
    ...
    res.json({
      ...
      recipient: {
        ...
        phone: order.recipient_phone ? maskPhone(order.recipient_phone) : null,
      ...
      timeline: timeline,
      hasPickupCode: !!order.status && ['available', 'collected', 'delivered'].includes(order.status)
    });
  } catch (err) {
 ...
 * Verify pickup code for an order
 */
router.post('/:token/verify-pickup', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      SELECT pickup_code FROM orders WHERE qr_token = $1
    `, [token]);
