...
async function refundPaypalOrder({ orderId, amountEur, reason, adminUser, paypal, db }) {
  ...
  // Dériver le montant KMF à partir du taux de change de la commande
  ...
  // ── INSERT refunds (manquait avant ce patch) ────────────────────────────
  // BUG précédent : refundPaypalOrder n'insérait jamais dans la table refunds
  // → aucune trace comptable, aucun reçu possible, impossible de savoir si
  //   le remboursement était Stripe ou PayPal.
  ...
  if (refundRow?.id) {
    refundReceiptService.issue(refundRow.id, { issuedBy: adminUser?.id }).catch(err => {
      log.warn({ err, order_id: order.id }, '[payment-paypal] Émission reçu remboursement PayPal échouée (non-fatal)');
    });
  }
