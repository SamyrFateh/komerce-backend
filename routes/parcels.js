...
router.get('/debug/test-create', ...adminAgent, async (req, res) => {
  try {
    const { generateExternalCode, generateSealCode, logParcelEvent } = require('../services/parcel-security');
    const { generateParcelRef } = require('../utils/reference');
    
    // Step by step
    const steps = [];
    
    steps.push('1. generateExternalCode');
    const ext = generateExternalCode();
    steps.push(`   → ${ext}`);
    
    steps.push('2. generateSealCode');
    const seal = generateSealCode();
    steps.push(`   → ${seal}`);
    
    steps.push('3. generateParcelRef');
    const ref = await generateParcelRef(db);
    steps.push(`   → ${ref}`);
    
    // Get an order
    steps.push('4. get order');
    const { rows: orders } = await db.query('SELECT id FROM orders LIMIT 1');
    if (!orders.length) {
      steps.push('   → NO ORDERS');
      return res.json({ steps });
    }
    const order_id = orders[0].id;
    steps.push(`   → ${order_id}`);
    
    steps.push('5. INSERT parcel');
    const { rows } = await db.query(`
      INSERT INTO parcels (reference, external_code, seal_code, order_id, type, notes, status)
      VALUES ($1, $2, $3, $4, 'standard', NULL, 'draft')
      RETURNING id, reference, external_code, seal_code
    `, [ref, ext, seal, order_id]);
    steps.push(`   → ${JSON.stringify(rows[0])}`);
    
    steps.push('6. logParcelEvent');
    const evt = await logParcelEvent(db, {
      parcel_id: rows[0].id,
      event_type: 'created',
      actor_id: req.user.id,
      notes: 'Debug test',
    });
    steps.push(`   → ${JSON.stringify(evt)}`);
    
    // Cleanup
    await db.query('DELETE FROM parcel_events WHERE parcel_id = $1', [rows[0].id]);
    await db.query('DELETE FROM parcels WHERE id = $1', [rows[0].id]);
    steps.push('7. cleanup OK');
